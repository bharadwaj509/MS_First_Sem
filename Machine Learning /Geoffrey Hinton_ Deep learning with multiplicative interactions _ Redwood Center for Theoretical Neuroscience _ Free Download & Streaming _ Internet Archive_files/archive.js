...
(function( $ ) {
  ...
  AJS.search = function(lnk){
    ...
    // Dont make the (typically) open/showing tooltip show while animating.
    // In fact, let's remove it altogether bec. they presumably now know
    // what this does!
    $('#nav-search > a').tooltip('hide').tooltip('disable');

    if (AJS.aboutHide)
      AJS.aboutHide();
      
    // hide search on browser resize or orientation changes
    $(window).on('resize  orientationchange', function(evt){
      $.doTimeout('AJS.search' + evt.type, 250, function(){
        // avoid android kicking a resize event our way due to keyboard sliding up for search field entry!!
        if (evt.type=='resize'  &&  AJS.isTouchDevice())
          return;
        $('#nav-search-x').click();
      })
    });
    
    var fades = '.navbar-nav .dropdown-ia:not(#nav-search), .navbar-nav .navbar-brand, #beta';
    
    var $ns=$('#nav-search'); // the search glass icon div
    if ($ns.css('position')!='absolute'){

      // Due to the special "About" hovering, there is **usually** extra *asymmetric* left padding
      // to block the hover trigger in the "empty space" of the nav.
      // So temporarily make padding "normal" to make calc-ing where to move the glass *and* avoiding
      // the glass left padding possibly interfering with ".navbar-brand" ideal.
      $ns.css('padding-left', $ns.css('padding-right'));
      var lnow=$ns.offset().left;
      var wantedFormWidth = 300;
      var lnew=Math.max(10, lnow - wantedFormWidth);
      var wform = lnow-lnew;
      log('moving glass from lnow:'+lnow+' to lnew:'+lnew+' ('+(wform)+' pixels)');
      wform = Math.max(100, wform);
      $ns.css({position:'absolute', left:lnow, 'z-index':1});

      // now, per david, IFF the search form *can fit* in the nav bar, dont hide the other nav elements.
      // So check if we'd fit...
      var $logo = $('.navbar-nav .navbar-brand');
      var rlogo = $logo.offset().left + $logo.outerWidth();
      if (lnew <= rlogo)
        $(fades).fadeOut('slow'); // won't fit!
      
      setTimeout(function(){ $ns.attr('data-lwas',lnow).addClass('slidey').css({left:lnew}); },100);

      var lform = 50 + lnew;
      
      $('#nav-search-in input[type=text]').css({left:lform, width:wform});
      $('#nav-search-x').css({left:lform + wform - 1});

      setTimeout(function(){ $('#nav-search-in').fadeIn(); 
                             $('#nav-search-in input[type=text]').focus(); },1200);
      setTimeout(function(){ $('#nav-search-in input[type=text]').focus(); },2000);
      
      $('#nav-search-x').one('click', function(){
        // cancel search -- close and revert nav
        var lwas=parseInt($ns.attr('data-lwas'),10);
        //log(lwas);
        $ns.css({left:lwas}); // slide it back to where it was
        $('#nav-search-in').fadeOut();
        setTimeout(function(){
          $ns.removeClass('slidey').css({position:'relative', left:'', 'padding-left':''});
          $(fades).fadeIn('slow');
        },1200);
      });
    }
    else{
      // search-in-nav now has two ways to exit, by user clicking submitting search, or by hitting [x]
      if ($('#nav-search-in input[type=text]').val()){
        // user has typed text to search box type-in -- submit search
        $('#nav-search-in input[type=submit]').click();
      }
      else{
        $('#nav-search-in input[type=text]').attr({'placeholder':'click here and type your search'});
      }
    }
    return false;
  };
